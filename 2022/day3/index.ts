...
class Day3 extends AdventOfCode {
	protected input: Array<Array<string>> = [];
	...
	protected processInput(inputPath: string, resultPath: string): void {
		this.input = fs.readFileSync(inputPath, 'utf8')
			.split('\n')
			.map(line => {
				const item1 = line.trim().slice(0, line.length/2);
				const item2 = line.trim().slice(line.length/2, line.length);
				return [item1, item2];
		});
	...
	protected part1(): number {
		return this.input.reduce((acc, rucksack) => {
			const commonChar = rucksack[0].split('').find((a) => rucksack[1].split('').find(b => b === a)) || '';
	...
	protected part2(): number {
		...
		for (let i=0; i < this.input.length; i+=3){
			group.push(this.input.slice(i, i+3).map(input => input.join('')));
		}
		return group.reduce((acc, elf) => {
			const commonChar = elf[0].split('').find((a) => elf[1].split('').find(b => b === a) && elf[2].split('').find(c => c === a)) || '';
			return acc += this.getPrio(commonChar);
		}, 0);
	}
}
